...
cleanup() {
  ...
  t0=$(start_timer)
  cmd=( "${LOCATIONS[script]}/execute_tidy_up.sh" "test" --output "$outputDir" --submitScript "$submitScript" -v "$flag_verbose" --skip 0,1,3)
...
main(){
  ...
  scriptPath="${LOCATIONS[script]}/all_testcases.sh"
  layer=2
  ...
  outputDir="$(get_path_outputRootDir "${LOCATIONS[output]}" "$layer" "-" "-")"
  mkdir -p "$outputDir/log/"

  submitScript="$(get_name_submitFile "submit_test" $layer "-" "-" "slurm")"
  jobName="$(get_name_job "test" "$layer" "-" "-")"
...
EOF
  ...
  cmd=("$scriptPath" --steps "${steps}" --process "${process}" -v "${flag_verbose}" --layer "$layer")
  [[ "$flag_color" == true ]] && cmd+=( --color )
  [[ -n "${completed_modes[*]}" ]] && cmd+=( --select "$(IFS="${SEPARATOR[list]}"; echo "${completed_modes[*]}")" )
